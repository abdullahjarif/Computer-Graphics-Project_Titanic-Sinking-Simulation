...
bool isMoving = false;
bool isDaytime = true; // Variable to track day or night
...
void Sky(){
     ...
     if (isDaytime) {
        ...
        glColor3ub(102, 212, 238); // Daytime sky color
        // Draw the sky polygon
        ...
        glBegin(GL_TRIANGLE_FAN);
        glColor3ub(10, 10, 50); // Nighttime sky color
        // Draw the sky polygon
            glVertex2f(0.375f, -0.161f);
            glVertex2f(0.542f, -0.153f);
            glVertex2f(0.540f, -0.232f);
...
void Water(){
      ...
      if (isDaytime) {
        glBegin(GL_TRIANGLE_FAN);
        glColor3ub(47, 136, 220); // Daytime water color
        ...
        glBegin(GL_TRIANGLE_FAN);
        glColor3ub(20, 20, 40); // Nighttime water color
        // Draw the water polygon
             glVertex2f(-0.682f, -0.456f);
            glVertex2f(0.325f, -0.452f);
            glVertex2f(0.194f, -0.997f);
...
// Global variables for cloud movement
float cloudPosX = -0.5f; // Initial x-position of the cloud
float cloudPosY = 0.7f;  // Initial y-position of the cloud
float cloudSpeedX = 0.001f; // Speed of the cloud movement in x-direction
float cloudSpeedY = 0.0f;   // Speed of the cloud movement in y-direction

void Cloud() {
    glColor3ub(200, 220, 220);
    drawCircle(cloudPosX, cloudPosY, 0.2f);  // Example cloud
    drawCircle(cloudPosX + 0.2f, cloudPosY + 0.1f, 0.15f);
    drawCircle(cloudPosX, cloudPosY, 0.1f);
    drawCircle(cloudPosX + 0.8f, cloudPosY - 0.1f, 0.15f);  // Another example cloud
    drawCircle(cloudPosX + 1.1f, cloudPosY + 0.1f, 0.1f);   // Third cloud
    drawCircle(cloudPosX + 1.0f, cloudPosY, 0.2f);   // Third cloud
}
...
void keyboard(unsigned char key, int x, int y) {
    switch (key) {
        ...
        case 'd':
        case 'D':
            isDaytime = !isDaytime;// Toggle day or night
            break;
    }
...
void timer(int) {
    ...
    cloudPosY += cloudSpeedY;

    // Wrap the cloud around when it goes off-screen
    if (cloudPosX > 1.2f)
        cloudPosX = -0.5f;
    if (cloudPosY > 1.0f)
        cloudPosY = -0.5f;
